...
- [ ] ForestTree 组件是否可递归渲染任意深度？→ 是，`TreeNode` 自引用
- [ ] 是否支持 STREAM（实时展开）和 SUMMARY（卡片+展开）两种模式？→ 是
- [ ] 节点状态是否有不同图标区分？→ 是，6 种状态图标
- [ ] SSE 连接是否在完成后自动关闭？→ 是
- [ ] 所有组件是否为纯自定义（无第三方 UI 库）？→ 是
- [ ] 简单对话模式是否隐藏树视图？→ 是，MessageNode 单节点不展示 ForestTree
- [ ] chat ↔ tree 视图切换是否无损？→ 是，展开/收起基于同一数据源
- [ ] 新手引导是否默认 chat 模式？→ 是，直到用户首次手动展开树
