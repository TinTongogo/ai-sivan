...
| # | 检查项 | 状态 | 说明 |
|---|--------|------|------|
| 1 | 所有节点输出结构化日志 | ❌ | `StructuredLogging` 类未实现，日志为文本格式 |
| 2 | 关键业务路径有 Prometheus 指标 | ✅ | `PrometheusMetricsService` + `micrometer-registry-prometheus` + management 端点配置 |
| 3 | LLM 调用耗时和 token 消耗被追踪 | ✅ | `MetricsDecorator`/`MetricsModel`（进程内存）+ `PrometheusMetricsService`（Micrometer） |
| 4 | GoalTree 执行可完整追溯每个节点 | ⚠️ | `SpanContext.child()` 现已正确创建子 Span 层级，但导出仍为同步 |
| 5 | 可观测性组件故障不影响主流程 | ⚠️ | Span 导出同步但通过 try-catch 静默；Micrometer 指标不影响主流程 |
| 6 | 结构化 JSON 日志 | ❌ | 未实现，需 logstash-logback-encoder 依赖 |
| 7 | Reactor Context 传播 traceId | ❌ | 仍使用 ThreadLocal，线程切换时 traceId 可能丢失 |
| 8 | 采样策略（按 mode 分频） | ✅ | `shouldSample()` — CONDITIONAL/CONSENSUS 100%、PARALLEL 10%、SEQUENTIAL/HIERARCHICAL 50% |

### 实现文件

| 组件 | 路径 |
|------|------|
| `PrometheusMetricsService` | `sivan-web/.../observability/PrometheusMetricsService.java` |
| `micrometer-registry-prometheus` 依赖 | `sivan-web/pom.xml` |
| management 端点配置 | `sivan-web/src/main/resources/application.yml` |
| Span 层级与采样（ForestExecutor） | `sivan-infra/.../forest/execution/ForestExecutor.java` |
