...
| # | 检查项 | 状态 | 说明 |
|---|--------|------|------|
| 1 | 每个 ModeStrategy 有单元测试 | ⚠️ | `SequentialModeStrategyTest` + `ConsensusModeStrategyTest` 已实现 |
| 2 | 每个 Policy 有正常+拒绝路径测试 | ⚠️ | `FileSecurityManagerTest` 覆盖 11 种场景 |
| 3 | 每个 ProgressStrategy 有进度计算测试 | ✅ | `ProgressAggregatorTest` 覆盖 9 种场景 |
| 4 | CTE 递归查询有深树性能测试 | ❌ | 无 1000 层深树 SQL 种子文件 |
| 5 | 端到端测试覆盖全链路 | ⚠️ | `ForestGoalE2ETest` + `ConversationE2ETest` |
| 6 | 测试可本地运行 | ✅ | `MockContinuation` 可复用 |

### 新增文件

| 文件 | 说明 |
|------|------|
| `sivan-agent/src/test/.../MockContinuation.java` | 共享 Mock，供所有 ModeStrategy 测试使用 |
| `sivan-agent/src/test/.../SequentialModeStrategyTest.java` | 顺序模式策略测试（顺序执行、跳过已完成、空子节点） |
