...
class GoalExecutionService {
    ...
    /**
     * 原子化完成一个节点：
     * 1. 更新节点 status → COMPLETED
     * 2. 持久化到 DB
     * 3. 发布 NodeStatusChanged 事件
     *
     * 三者在一个事务中。任何一步失败 → 全部回滚 → status 保持 PENDING。
     */
    public void completeNode(ExecutableNode node, UUID forestId, UUID accountId) {
        node.setStatus(NodeStatus.COMPLETED);
        forestRepo.saveNode(node);

        events.publishEvent(new NodeStatusChanged(
            node.nodeId(), node.nodeType(), null, NodeStatus.COMPLETED,
            forestId.toString(), Instant.now()
        ));
    }
}
```
...
- [ ] 节点 status 变更是否是事务性的？→ 是，`@Transactional`
- [ ] 启动时是否自动恢复 RUNNING 状态的 GoalTree？→ 是，`RecoveryManager`
- [ ] 事件发布是否在事务内，失败是否回滚？→ 是，同步模式
- [ ] 异步订阅者失败是否不影响主流程？→ 是，`AFTER_COMMIT`
- [ ] 递归执行异常是否设置节点 FAILED？→ 是，`doOnError`
- [ ] 所有 Repository 方法是否带 accountId？→ 是
