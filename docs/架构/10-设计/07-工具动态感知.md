...
| # | 检查项 | 状态 | 实现组件 | 位置 |
|---|--------|------|----------|------|
| 1 | 新增工具来源需改几个文件 | ✅ | `ToolProvider` 接口 | `sivan-core-api/.../core/tool/ToolProvider.java` |
| 2 | 新增路由策略需改几个文件 | ✅ | `ToolRoutingStrategy` 接口 | `sivan-agent/.../routing/ToolRoutingStrategy.java` |
| 3 | MCP 服务器运行时热加载 | ✅ | `ToolProviderManager.connectMcpServer()` | `sivan-agent/.../tool/ToolProviderManager.java` |
| 4 | 工具列表缓存 | ✅ | `ToolCache` (TTL 30s + 手动失效) | `sivan-agent/.../tool/ToolCache.java` |
| 5 | 内部/外部工具统一接口 | ✅ | 都实现 `ToolProvider` | `sivan-core-api/.../core/tool/ToolProvider.java` |
| 6 | 工具经过沙箱校验 | ✅ | `ToolExecutor` → `SandboxManager.execute()` | 现有实现 |
| 7 | 文件操作路径安全校验 | ✅ | `FileSecurityManager.validate()` + 禁止列表 + 写目录限制 | `sivan-infra/.../file/FileSecurityManager.java` |
| 8 | 内部工具不暴露外部 MCP 协议 | ✅ | 通过 `ToolRegistryImpl` 内部注册 | 现有实现 |
| 9 | 项目目录自动初始化 | ✅ | `ProjectService.create()` 自动创建 data/output/uploads | 现有实现 |
| 10 | MCP 配置 CRUD | ✅ | `McpServerConfigController` 完整增删改查 + 连接/断开/工具列表/预检 | `sivan-web/.../tool/controller/McpServerConfigController.java` |
| 11 | MCP 连接状态监控 | ✅ | 每 60 秒 `@Scheduled` 心跳检测 + 连续 3 次失败自动断开 | `McpConnectionManager.heartbeat()` |
| 12 | OAuth 凭证加密存储 | ✅ | `ApiKeyEncryptor` 加解密 | 现有实现 |
| 13 | 连接按 accountId 隔离 | ✅ | `ToolProviderManager` 维护 `Map<accountId, Map<serverId, ConnectionState>>` | `sivan-agent/.../tool/ToolProviderManager.java` |
| 14 | McpServerConfig 连接状态字段 | ✅ | connectionStatus / lastError / lastConnectedAt / toolCount + DB 迁移 V20 | `McpServerConfig.java` + `V20__*.sql` |
| 15 | 工具路由策略模式 | ✅ | `ToolRouter` + `AgentToolStrategy` / `HomeToolStrategy` / `DefaultToolStrategy` | `sivan-agent/.../routing/` |
| 16 | 工具预检 | ✅ | `ToolPreflight.check(serverId)` + `PreflightResult` | `sivan-agent/.../tool/ToolPreflight.java` + `sivan-domain/.../tool/PreflightResult.java` |
| 17 | AgentLeafExecutor 集成 ToolRouter | ✅ | 注入 `ToolRouter`，按叶子类型路由工具 | `sivan-agent/.../forest/AgentLeafExecutor.java` |
