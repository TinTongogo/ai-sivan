...
export interface ForestTreeNode {
  name: string
  status: 'COMPLETED' | 'RUNNING' | 'PENDING' | 'FAILED' | 'CANCELLED' | 'SKIPPED'
...
<template>
  <div class="forest-tree-node" :style="{ paddingLeft: depth * 20 + 'px' }">
    ...
    >
      ...
      <!-- Status icon -->
      <span class="forest-tree-status">{{ statusIcon }}</span>
      ...
      <!-- Mode label -->
      <span v-if="modeLabel" class="forest-tree-mode" :title="node.mode">{{ modeLabel }}</span>
      ...
      <!-- Status badge -->
      <span v-if="node.isLeaf || !isExpandable" class="forest-tree-badge" :class="'forest-tree-badge--' + node.status.toLowerCase()">
        {{ node.status === 'COMPLETED' ? '✅' : node.status === 'RUNNING' ? '🔄' : node.status === 'PENDING' ? '⏳' : '' }}
      </span>
    </div>
